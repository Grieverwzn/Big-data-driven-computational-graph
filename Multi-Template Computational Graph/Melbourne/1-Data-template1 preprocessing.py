...
"""
# Description: Template 1 data preprocessing: event-based demand generation
...
EXTRA_CENTROIDS = list(map(int, STADIUM_NODE_IDS + POI_NODE_IDS))  # 合并列表
...
for i in trange(len(Centroid)):
    ...
    if cen not in link_from_node_id:
        ...
        # Compute the distance between cen and each link starting node
    ...
    if cen not in link_to_node_id:
        ...
        # Compute the distance between cen and each link ending point
...
link_mean_len = link.loc[link['length'] > 0, 'length'].mean() if (link['length'] > 0).any() else 1.0
raw_len = link_add.apply(lambda r: length(int(r['from_node_id']), int(r['to_node_id'])), axis=1)
link_add['length'] = np.where(raw_len > 0, raw_len, link_mean_len).clip(min=1e-6)


CONNECTOR_FF_TT = 1000.0
link_add['free_flow_travel_time'] = CONNECTOR_FF_TT


fs = link_add['length'] / (CONNECTOR_FF_TT / 60.0)
link_add['free_speed'] = fs.replace([np.inf, 0], np.nan).fillna(link['free_speed'].median())


link_add['capacity'] = float(link['capacity'].median()) * 10.0
link_add['lanes']    = float(link['lanes'].median()) if link['lanes'].notna().any() else 1.0
# =========================================
...
if 'free_flow_travel_time' not in link_last.columns:
    link_last['free_flow_travel_time'] = np.nan
need = link_last['free_flow_travel_time'].isna()
link_last.loc[need, 'free_flow_travel_time'] = link_last.loc[need, 'length'] / link_last.loc[need, 'free_speed'] * 60.0
...
#link_last = link_last.fillna(0)
link_last




link_last.to_csv('data_1/link_last.csv',index=False)

link_last = pd.read_csv('data_1/link_last.csv')
link_last
...
for i in ob_unique:
    ...
    #flow0.loc[flow0['link_ID']==link_unique[i], 'link_ID'] = i+1
    #link.loc[link['link_id']==link_unique[i], 'link_id'] = i+1
...
DataFrame(node_unique).to_csv('data_new_1/node_unique.csv',index=False)
...
for k, ev in enumerate(EVENTS):
    ...
    for j, w in enumerate(ARRIVE_WEIGHTS, start=1):
        tt = t0 - (len(ARRIVE_WEIGHTS) - j + 1)  # -> t0-3, t0-2, t0-1
        if 0 <= tt < T:
            W_ARR[k, tt] = w

    for j, w in enumerate(DEPART_WEIGHTS):
        tt = t_end + j                           # -> t_end+0, t_end+1, ...
        if 0 <= tt < T:
            W_DEP[k, tt] = w
...
for o in Z:
    ...
    for d in reach.intersection(set(Z)):
        reach_pairs.add((o, d))
mask_reach = np.array([(int(OD_pair[i,0]), int(OD_pair[i,1])) in reach_pairs
                       for i in range(OD_pair.shape[0])])
print(f"[Reachability] 总OD: {OD_pair.shape[0]} | 可达: {mask_reach.sum()} | 不可达: {(~mask_reach).sum()}")
...
W_ARR_m = np.zeros((len(EVENTS_MASK), T), dtype=float)
W_DEP_m = np.zeros((len(EVENTS_MASK), T), dtype=float)
for k, ev in enumerate(EVENTS_MASK):
    t0 = int(ev["t_event"])
    if "duration_cols" in ev and pd.notna(ev["duration_cols"]):
        dur_cols = int(ev["duration_cols"])
    elif "duration_min" in ev and pd.notna(ev["duration_min"]):
    ...
    for j, w in enumerate(ARRIVE_WEIGHTS, start=1):
        tt = t0 - (len(ARRIVE_WEIGHTS) - j + 1)  # t0-3, t0-2, t0-1
        if 0 <= tt < T: W_ARR_m[k, tt] = w
    for j, w in enumerate(DEPART_WEIGHTS):
        tt = t_end + j                           # t_end+0, t_end+1
        if 0 <= tt < T: W_DEP_m[k, tt] = w

    if W_ARR_m[k].sum() > 0: W_ARR_m[k] /= W_ARR_m[k].sum()
    if W_DEP_m[k].sum() > 0: W_DEP_m[k] /= W_DEP_m[k].sum()


demand_cols_mask = []
for t_idx, path in enumerate(file_path):
    M = pd.read_csv(path, index_col=0).iloc[:-1, :-1]   # 背景OD
    M_mask = pd.DataFrame(0.0, index=rows, columns=cols)
    M_mask.loc[M.index, M.columns] = M.values
    ...
    for k, ev in enumerate(EVENTS_MASK):
        ...
        default_car = float(STADIUM_META[sid]["car_ratio"])
        att_raw = ev.get("attend", None)
        att = cap if (att_raw is None or (isinstance(att_raw, float) and np.isnan(att_raw))) else float(att_raw)
        car_raw = ev.get("car_ratio", None)
        car = default_car if (car_raw is None or (isinstance(car_raw, float) and np.isnan(car_raw))) else float(car_raw)
        if car > 1.0: car /= 100.0
        Qtot = att * car
        wA, wD = W_ARR_m[k, t_idx], W_DEP_m[k, t_idx]
        ...
        if wD > 0 and Qtot > 0:
            ...
            if (not np.isfinite(d2).all()) or (np.max(d2) <= 0):
                ...
                p2 = np.exp(-THETA * (d2 / np.max(d2))); p2 /= p2.sum()
            for dz, add in zip(Dst, Qtot * wD * p2):
                M_mask.loc[str(sid), str(dz)] += add


    for a in ATTRACTIONS_MASK:
        sid  = int(a["sid"])
        vis  = float(a.get("visitors_am", 0) or 0)
        carr = float(a.get("car_ratio", 0.4) or 0.4)
        if carr > 1.0: carr /= 100.0
    ...
    demand_cols_mask.append(M_mask.values.reshape(-1, 1))

demand_mask = np.hstack(demand_cols_mask)     # (num_od^2, T)
idx_mask = demand_mask.sum(axis=1) > 0
OD_pair = OD_pair[idx_mask, :]
demand  = demand[idx_mask, :]
...
link.to_csv('data_1/link.csv',index=False)
...
node.to_csv('data_1/node.csv', index = False)


OD_pair = DataFrame(OD_pair)
OD_pair.columns = ['o_zone_id', 'd_zone_id']
OD_pair.to_csv('data_1/OD_pair.csv', index = False)
...
DataFrame(demand).to_csv('data_1/demand_6-10.csv', index=False)


flow.to_csv('data_1/link_flow.csv', index = False)


tmp_node = pd.read_csv('data_new_1/node.csv')
tmp_link = pd.read_csv('data_new_1/link.csv')
nset = set(tmp_node['node_id'].astype(int))
lset = set(tmp_link['from_node_id'].astype(int)) | set(tmp_link['to_node_id'].astype(int))
print("node covers all link endpoints?", lset.issubset(nset))